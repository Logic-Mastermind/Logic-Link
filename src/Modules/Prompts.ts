...
//         let category = await client.functions.findCategory(msgArgs.join(" "), msg.guild);
//         if (!category) {
...
//           let role = await client.functions.findRole(arg, msg.guild);
...
//       if (finished) {
//         let categoryOpened = message.guild.channels.cache.get(collected.opened);
//         let categoryClosed = message.guild.channels.cache.get(collected.closed);
...
//             let category = await client.functions.findCategory(msgArgs.join(" "), m.guild);
//             if (!category) {
//               const embed = client.embeds.error(title[current], `I could not record any categories from your message, please try again.`, originalQuestion);
//               return editMsg.edit({ embeds: [embed] });
//             }
...
//             for await (const arg of args) {
//               let role = await client.functions.findRole(arg, m.guild);
//               if (role) roles.push(role.id);
//             }
...
//           let panelChannel = await client.channels.fetch(panelInfo.channel).catch(() => {});
...
//         try {
//           let panelChannel = await client.channels.fetch(panel.channel);
